...
SemaphoreHandle_t usdhc_semphore;
static volatile uint32_t s_usdhc_int_status;

static uint8_t usdhc_get_instance(imx_usdhc_t *usdhc)
{
    uint32_t instance = 1;
    if (usdhc == 0x02190000) {
        instance = 1;
...
bool usdhc_send_command(void *host, uint8_t cmd_idx, uint32_t arg)
{
    ...
    if (cmd.dma_en == false) {
        ...
        usdhc->int_singal_en |= (USDHC_INT_SIGNAL_EN_DMAEIEN_MASK |
    ...
    xSemaphoreTake(usdhc_semphore, portMAX_DELAY);

    /* check CCE or CTOE error */
    if ((s_usdhc_int_status & USDHC_INT_STATUS_CCE_MASK) != 0) {
        ret = false;
        USDHC_TRACE("%s Command CRC error\n", __func__);
        goto cleanup;
    }

    if ((s_usdhc_int_status & USDHC_INT_STATUS_CTOE_MASK) != 0) {
        ret = false;
        USDHC_TRACE("%s Command Timeout error\n", __func__);
        goto cleanup;
    }

    s_usdhc_int_status = 0;
    /* mask all the interrupt signal */
...
static void usdhc_irq_handler()
{
    imx_usdhc_t *usdhc = (imx_usdhc_t *)0x02190000;
    s_usdhc_int_status = usdhc->int_status;
    usdhc->int_status |= 0xffffffff; /* clear all interrupt */
    xSemaphoreGiveFromISR(usdhc_semphore, NULL);
}
...
bool usdhc_init(void *host)
{
    ...
    usdhc_semphore = xSemaphoreCreateCounting(1, 0);
    if (usdhc_semphore == NULL) {
        USDHC_TRACE("create usdhc_semphore failure\n");
        while(1);
    } else {
        USDHC_TRACE("usdhc_semphore:%x\n", usdhc_semphore);
    }

    if (usdhc_get_instance(usdhc) == 1) {
        GIC_SetPriority(USDHC1_IRQn, 25);
        EnableIRQ(USDHC1_IRQn);
        SystemInstallIrqHandler(USDHC1_IRQn, 
            (system_irq_handler_t)usdhc_irq_handler, NULL);
    } else {
        USDHC_TRACE("Not supported yet");
        return false;
    }
